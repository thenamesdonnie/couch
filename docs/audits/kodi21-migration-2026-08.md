...
| 8 | **`kodi-tv`'s crash-relaunch through `flatpak run` — now CLOSED.** The loop keys on exit codes 131–136/139; the in-sandbox `kodi.sh` does `exit $RET` and `flatpak run` has to pass it back. `server/sys.js`'s hardened restart rides the same path, so the whole no-stranded-TV story rests on it. Raised as a [B] because confirming it means deliberately crashing the live Kodi — then confirmed anyway, on a restart the evening needed for other reasons. | B → **closed** | VERIFIED the same evening, riding a restart that was needed anyway: `POST /api/system/kodi-restart` (i.e. `server/sys.js` SIGABRTing kodi.bin) and the wrapper brought Kodi back through `flatpak run` in ~6s, still supervising. The exit code propagates |
