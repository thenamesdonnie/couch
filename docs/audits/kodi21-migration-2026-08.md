...
| 8 | **`kodi-tv`'s crash-relaunch is unverified through `flatpak run`.** The loop keys on exit codes 131–136/139. The in-sandbox `kodi.sh` does `exit $RET`, and `flatpak run` should propagate it, but this was never observed — and it is also what `server/sys.js`'s hardened restart depends on. Untestable without deliberately crashing the live Kodi. | B | → todo.md. One `Application.Quit`-adjacent test next time Kodi is being restarted anyway |
