        ...
        for episode in json_query:
                ...
                if use_last_played_season:
                    episode_query = json_call('VideoLibrary.GetEpisodes',
                                              ...
                                              query_filter={'and': [self.filter_unwatched, {'field': 'inprogress', 'operator': 'false', 'value': ''}, {'field': 'season', 'operator': 'is', 'value': str(last_played_query['result']['episodes'][0].get('season'))}]},
                                              params={'tvshowid': int(episode['tvshowid'])}
                                              )
                ...
                if not use_last_played_season:
                    episode_query = json_call('VideoLibrary.GetEpisodes',
                                              properties=JSON_MAP['episode_properties'],
                                              sort={'order': 'ascending', 'method': 'episode'}, limit=1,
                                              query_filter={'and': [self.filter_unwatched, {'field': 'inprogress', 'operator': 'false', 'value': ''}, self.filter_no_specials]},
                                              params={'tvshowid': int(episode['tvshowid'])}
                                              )
